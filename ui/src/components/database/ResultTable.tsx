  ...
  return (
    <div style={{ fontSize: 11, overflow: 'auto', height: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 8px', background: '#333', alignItems: 'center' }}>
        <span style={{ color: '#ccc' }}>{rows.length} 行 {columns.length} 列</span>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
          <thead>
            <tr>
              {columns.map((col) => (
                <th key={col} style={{
                  padding: '4px 8px', background: '#2d2d2d', color: '#4fc3f7',
                  borderBottom: '1px solid #444', textAlign: 'left', whiteSpace: 'nowrap',
